...
import ch.makery.address.model.Drone;
...
//leaves
	...
	@FXML private ImageView mountain2;
...
//buttons
	...
	@FXML private Button flyToLocation;
	@FXML private Button rotateDroneLeft;
	@FXML private Button rotateDroneRight;
	@FXML private Button deleteComp;
	@FXML private TextField name;
	@FXML private TextField Xpos;
	@FXML private TextField Ypos;
	@FXML private TextField droneXpos;
	@FXML private TextField droneYpos;
...
// SingleTon Stuff --------------------------------------------------------------------------------
	...
	private void addDrone() {
		...
		myDrone = new Drone("DJI Phantom 4 RTK",8419,v,0,0,50,50,50,c);
	...
	@FXML
	private void rotateDroneLeft() {
		if (myDrone != null) {
			myDrone.getImage().setRotate(myDrone.getImage().getRotate() - 90);
			myDrone.rotateLeft();
		}
	}
	@FXML
	private void rotateDroneRight() {
		if (myDrone != null) {
			myDrone.getImage().setRotate(myDrone.getImage().getRotate() + 90);
			myDrone.rotateRight();
		}
	...
	@FXML
	private void getDroneXPos() {
		if (myDrone != null) {
			String Xpos = Integer.toString(myDrone.getLocationX());
			droneXpos.setText(Xpos);
		}
	}
	
	@FXML
	private void getDroneYPos() {
		if (myDrone != null) {
			String Ypos = Integer.toString(myDrone.getLocationY());
			droneXpos.setText(Ypos);
		}
	}
	
	// Moves the Drone to a (x,y) based on a pictures location
	@FXML
	private void droneFlyToLocation() {
		System.out.println("Flying to (X,Y): " + droneXpos.getText() + ", " + droneYpos.getText());
		int goToX = Integer.parseInt(droneXpos.getText());
		int goToY = Integer.parseInt(droneYpos.getText());
		
		//adjust to ensure goTo values are within boundaries.
		if (goToX > 750) {goToX = 750;}
		if (goToY > 550) {goToY = 550;}
		
		// Distance to move
		int Xdist = Math.abs(goToX - myDrone.getLocationX());
		int Ydist = Math.abs(goToY - myDrone.getLocationY());
		
		// Time to destination in doubles
		double Xduration = Xdist / 150;
		double Yduration = Ydist / 150;
		
		// Go to X position.
		if(myDrone.getLocationX() < goToX) { // We need to go RIGHT
			if (myDrone.isDown()) {rotateDroneLeft();}
			else if (myDrone.isUp()) {rotateDroneRight();}
			else if (myDrone.isLeft()) {
				rotateDroneRight();
				rotateDroneRight();
			}	
			
		} else if(myDrone.getLocationX() > goToX) { // We need to go LEFT
			if (myDrone.isDown()) {rotateDroneRight();}
			else if (myDrone.isUp()) {rotateDroneLeft();}
			else if (myDrone.isRight()) {
				rotateDroneLeft();
				rotateDroneLeft();
			}
		}
		String strXdist = Integer.toString(Xdist);
		if(myDrone.isRight()) {
			System.out.println("Flying Right for " + strXdist + "units.");
		}
		if(myDrone.isLeft()) {
			System.out.println("Flying Left for " + strXdist + "units.");
		}
		
        TranslateTransition firstTransition =
                new TranslateTransition(Duration.seconds(Xduration), myDrone.getImage());
            firstTransition.setFromX(myDrone.getLocationX());
            firstTransition.setToX(goToX);
            firstTransition.setCycleCount(1);



		// Go to Y position.
		if(myDrone.getLocationY() < goToY) { // We need to go DOWN
			if (myDrone.isLeft()) {rotateDroneLeft();}
			else if (myDrone.isRight()) {rotateDroneRight();}
			else if (myDrone.isUp()) {
				rotateDroneRight();
				rotateDroneRight();
			}	
			
		} else if(myDrone.getLocationY() > goToY) { // We need to go UP
			if (myDrone.isLeft()) {rotateDroneRight();}
			else if (myDrone.isRight()) {rotateDroneLeft();}
			else if (myDrone.isDown()) {
				rotateDroneLeft();
				rotateDroneLeft();
			}
		}
		String strYdist = Integer.toString(Ydist);
		if(myDrone.isUp()) {
			System.out.println("Flying Up for " + strYdist + " units.");
		}
		if(myDrone.isDown()) {
			System.out.println("Flying Down for " + strYdist + " units.");
		}
		
        TranslateTransition secondTransition =
                new TranslateTransition(Duration.seconds(Yduration), myDrone.getImage());
            secondTransition.setFromY(myDrone.getLocationY());
            secondTransition.setToY(goToY);
            secondTransition.setCycleCount(1);
	
        SequentialTransition sequentialTransition = new SequentialTransition();
        sequentialTransition.getChildren().addAll(firstTransition, secondTransition);
        sequentialTransition.setCycleCount(1);
        sequentialTransition.play();
        
        myDrone.setLocationX(goToX);
        myDrone.setLocationY(goToY);
        System.out.println("Current Drone position(X,Y): " + myDrone.getLocationX() + ", " + myDrone.getLocationY());
		System.out.println();
	}
	...
	private void submitItem() {
		...
		if(isContainer.isSelected()) {
			...
			container.getImage().setOnMouseClicked(event ->{picX = container.getImage().getX();
	...
	private void enlargeImage() {
			...
			mountain2.setOnMouseClicked(event ->{windowView.setImage(mountain2.getImage());});
